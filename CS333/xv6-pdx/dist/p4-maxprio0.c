/*====================================================================*
 * This is a modfied version of maxpriority.c for testing MAXPRIO = 0 *
 * and for testing setpriority and getpriority functionality.
 * Created with permission by Jaime Landers on 5/28/19                *
 *===================================================================*/
...
// This tests the demotion facility.
void
testDemotion(void)
{
  printf(1, "\n\nTesting demotion...\n");
...
//  int startTicks;
  ...
  if (pid == 0) {
    printf(1, "Child process sleeping for 5 seconds, priority should not change. Use Ctrl-p to show this\n");
//    startTicks = ticks;
    sleep(5000);
//    while(1);
//    while(ticks < startTicks + 5000);
  } else {
    wait();
  }
}
...
testPromotion(void)
{
  ...
  if (newPrio != prio && newPrio > prio) {
    printf(1, "Promotion has occurred when it shouldn't.\n");
    printf(1, "**** TEST FAILED  ****\n\n\n");
  } else {
    printf(2, "Promotion did not occur.\n");
...
checkPriority(void)
{
  ...
  printf(1, "Testing setpriority() with valid PID and priority\n");
  printf(1, "Ctrl-p then Ctrl-r to show budget before setpriority()\n");
  sleep(2000);
//  int cpid = fork();
  ...
  printf(1, "Ctrl-p then Ctrl-r to show budget after setpriority()\n");
  sleep(2000);
  ...
  // Test that the priority cannot be set to a number other than 0 for MAXPRIO = 0:
/*  printf(1, "Testing that a priority cannot be set to a value other than 0 for MAXPRIO = 0.\n");
  printf(1, "  Testing setting priority to a positive number.\n");
  originalPriority = getpriority(pid);
  rc = setpriority(pid, 1);
  printf(1, "  setPriority(%d, 1) returned %d.\n", pid, rc);
  if (rc != 0) {
...
{
/*  printf(1, "\nWelcome to MAXPRIO = 0 Test\n");
  if (MAXPRIO != 0) {
    printf(1, "This test is for testing MAXPRIO = 0, set MAXPRIO = 0 and try again...\n");
    exit();
  } else {
    printf(1, "MAXPRIO = 0, commencing tests... \n");
  }
*/  checkPriority();
