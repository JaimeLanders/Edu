...
> between       :: Int -> Int -> Int -> Bool
> between p q r  = if p < r && r < q then True else False

> validHour     :: Int -> Bool
> validHour p    = if p < 12 && p >= 0 then True else False  

> validMinute   :: Int -> Bool
> validMinute p  = if p < 60 && p >= 0 then True else False  
...
> second'n'third :: [Int] -> (Int,Int)
> second'n'third x  = (head(tail x), head(tail(tail x))) 
...
> test3 = ['a', 'b', 'c', 'd', 'e']
> test4 = ["hello", "my", "name", "is", "Jaime"]

> third :: [a] -> a 
> third x1 = head (tail (tail x1)) 

(b)

> splice :: [a] -> [a] -> [a]
> splice p q = head p : tail q 
...
Since the defintion of foo takes two paramaters, a and b and returns a, there's
not a lot that can be done with that definition whether or not the types are the  
same or not.
...
> thirdChar st1 = head(tail(tail(st1))) 

> thirdString :: String -> Char 
> thirdString = thirdChar -- do not change anything to the left of the = sign!
...
>   = case t of
>       ILeaf -> ILeaf 
>       INode n left right -> INode (n * 2) (doubleITree left) (doubleITree right) 
...
>                 Leaf p -> 1
>                 Node p left right -> 1 + max (depthTree left) (depthTree right)
...
>       NotB e1       -> not (beval env e1)


> evalBProgram :: BExp -> Bool 
> evalBProgram bexp = beval emptyEnv bexp
...
> data Fahrenheit = F Float
>      deriving (Eq, Show)

> ftoc :: Fahrenheit -> Celsius 
> ftoc (F f) = (f - 32 * (5 / 9))

-->   deriving (Eq, Show) -- Syntax error
...
(a) This type of constructor is recursion since Trees use recursion.

     deriving (Eq, Show)

> data TwoThreeTree = TLeaf  
>                   | TNode1 Int TwoThreeTree TwoThreeTree
>                   | TNode2 Int Int TwoThreeTree
>   deriving (Eq, Show)

(b) Since the definition can contain either an empty bit vector, a single Bool
    or a combination of any two of these (and itself), I would argue that this
    is a Sum.

> data BoolVector = BEmpty  
>                 | BSingle Bool
>                 | BDouble BoolVector BoolVector
>   deriving (Eq,Show)

(c) Enumerations are types of Sums, so this constructor is a Sum.

> data Month = Jan | Feb | Mar | Apr | May | Jun | Jul | Aug | Sep | Oct | Nov | Dec  
>   deriving (Eq,Show)

(d) Since this construct packs multiple types into one collection accessible by
    name, it is a Record or a Product. 

> data FlightRecord = MkFlightRecord { num :: Int, start :: String, end :: String, avgprice :: Float }
>   deriving (Eq, Show)

(e) Since Temp can be F, C or both, then this constuctor is a Sum. 

> data Temps = Fah Float
>            | Cel Float
>   deriving (Eq, Show)

(f) This data type is a collection of two pairs of functions, which can be seen
    as a collection of tuples which are also collections, so this is in essence
...
I would argue that Java object types are records which are a subset of pro-
ducts.  The reason for this is that Java object can contain any number of
different types of data in one object, which is consistent with how records
are defined, and thus products.  Any of these data types are generally access-
able by name, which is another feature common of product types.  However, I can
also see an argument for Java objects being Sums since the values of the data
that can hold a null value won't be null and contain a value at the dame time,
which is an alternative.  Overall I feel that Java objects are more consistent
with how we have defined products.
...
If we were to represent Int's, I feel it would be best to do so using Products
since an integer with 29 bits can be seen as a Product of 29 base 2 digits. It
wouldn't make sense to me to represent an Integer as a Sum since it would take
a lot more space to represent the same amount of Integers as a product could
since there are alternatives.  For instance, there would need to be two sums
to represent the Integers 1 and 2, where one Product could easily represent
both. If we were to represent Integers we would need some way of identifying
them and couldn't just say this is an Int without specifying what type it is.
For this reason I believe that Haskell leaves some room in the 32 bits to store
or wrap the value with its identifier, or a pointer to the integer value.  
...
It's hard for me to tell as the S is throwing me off, but I believe this is the
set of Integers which can include both Integers (Z) and Natural Numbers (N).
I think the S might stand for Sum in this data declaration, which would make
sense since a Sum can be used to represent a Set including one of Z or N or 
both Z and N. 
